...
function DailyExpenseTableView() {
  ...
  const sorted = [...expenses].sort((a, b) => b.spentAt.localeCompare(a.spentAt));
  const total = sorted.reduce((sum, expense) => sum + (expense.amountInPen ?? expense.amount), 0);
  ...
  return (
    <div className="space-y-4">
      ...
      {sorted.length === 0 ? (
        <EmptyState description="No hay gastos del día a día en este mes" />
      ) : (
              ...
              {sorted.map((expense) => {
                ...
                return (
                  <TableRow key={expense.id}>
                    ...
                    <TableCell>
                      <CurrencyDisplay amount={expense.amount} currency={expense.currency} amountInPEN={expense.amountInPen} />
                    </TableCell>
